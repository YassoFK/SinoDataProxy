...
void  TcpOptDialog::showEvent(QShowEvent* event)
{
    QString ti[]={tr("TCP Server Options "),tr("TCP Client Options"),
                 ...
                 tr("NTRIP Caster Server Options"), tr("NTRIP Caster Client Options")};
    ...
    QString mountpoint=url.path();
    mountpoint.replace("/","");
    mountpoint.replace(":","");
    MntPnt->insertItem(0,mountpoint);MntPnt->setCurrentIndex(0);
    User->setText(url.userName());
    Passwd->setText(url.password());
    Str->setText(Str_Text);

    Addr->setEnabled((Opt>=1&&Opt!=4&&Opt!=5));
    MntPnt->setEnabled((Opt>=2&&Opt!=4));
    User->setEnabled(Opt==3||Opt==5);
    ...
    LabelUser->setEnabled(Opt==3||Opt==4||Opt==5);
...
//    MntPnt->clear();
    ...
    BtnNtrip->setVisible((Opt>=2&&Opt!=4&&Opt!=5));
}
//---------------------------------------------------------------------------
