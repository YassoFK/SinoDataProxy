...
#define PRGNAME     "SinoDataProxy"        // program name
...
// constructor --------------------------------------------------------------
MainForm::MainForm(QString name,QWidget *parent)
    : QDialog(parent)
{
    strname = name;
    ...
    QString temp = file.remove(".exe") + name;
    QFileInfo fi(temp);
    //fi.baseName()=name;
...
void MainForm::FormCreate()
{
    ...
    setWindowTitle(QString("%1").arg(strname));
...
{
    SvrStart();
    QString strinput = Input->currentText();
    QString stroutput1 = Output1->currentText();
    QString stroutput2 = Output2->currentText();
    QString stroutput3 = Output3->currentText();

    connectedit edit;
    edit.input = strinput;
    edit.name = strname;
    edit.output1 = stroutput1;
    edit.output2 = stroutput2;
    edit.output3 = stroutput3;
    emit sendInputStr(edit);
}
// callback on button-stop --------------------------------------------------
void MainForm::BtnStopClick()
{
    emit sendStopStatus(strname);
    SvrStop();
}
...
void MainForm::BtnOptClick()
{
    ...
    //for (int i=0;i<7;i++) SvrOpt[i]=svrOptDialog->SvrOpt[i];
...
void MainForm::GetNameStr(QString name)
{
    //strname = name;
}
...
void MainForm::Timer1Timer()
{
    ...
    int stat[4]={0},byte[4]={0},bps[4]={0}, log_stat[4]={0};
    ...
    num2cnum(byte[0],s1);
    num2cnum(bps[0],s2);
    QString strbyte = QString(s1);
    QString strbps = QString(s2);
    for (int i=0;i<4;i++) {
        num2cnum(byte[i],s1);
        num2cnum(bps[i],s2);
    ...
    QString strtime = QString("%1d %2:%3:%4").arg(t[0],0,'f',0).arg(t[1],2,'f',0,QChar('0')).arg(t[2],2,'f',0,QChar('0')).arg(t[3],2,'f',2,QChar('0'));
    ConTime->setText(strtime);

    QString strmessage = QString(msg);
    emit sendTime(strname,strtime,strbyte,strbps,strmessage);
...
void MainForm::SvrStart(void)
{
    ...
    int itype[]={
        STR_SERIAL,STR_TCPCLI,STR_TCPSVR,STR_NTRIPCLI,STR_FILE,STR_FTP,STR_HTTP,STR_NTRIPCAS
    };
    int otype[]={
        STR_NONE,STR_SERIAL,STR_TCPCLI,STR_TCPSVR,STR_NTRIPSVR,STR_FILE,STR_NTRIPCAS
    };
    //int ip[]={0,1,1,1,2,3,3},strs[4]={0},opt[7]={0},n;
    int ip[]={0,1,1,1,2,3,3,1},ip2[]={0,1,1,1,2,1},strs[4]={0},opt[7]={0},n;
    ...
    strcpy(paths[1],!Output1->currentIndex()?"":qPrintable(Paths[1][ip2[Output1->currentIndex()-1]]));
    strcpy(paths[2],!Output2->currentIndex()?"":qPrintable(Paths[2][ip2[Output2->currentIndex()-1]]));
    strcpy(paths[3],!Output3->currentIndex()?"":qPrintable(Paths[3][ip2[Output3->currentIndex()-1]]));
...
//    }
    ...
    // stream server start
    if (!strsvrstart(&strsvr,opt,strs,paths,logs,conv,cmd,cmds_periodic,AntPos)) return;
...
void MainForm::LoadOpt(void)
{
    ...
    //SvrOpt[7]=settings.value(QString("set/svropt_%1").arg(7),optdef[7]).toInt();
    for (int i=0;i<3;i++) {
...
void MainForm::SaveOpt(void)
{
    ...
    //settings.setValue(QString("set/svropt_%1").arg(7),SvrOpt[7]);
    for (int i=0;i<3;i++) {
